...
<c:if test="${requestScope.reservedOrders.size() > 0}">
...
<c:if test="${requestScope.reservedOrders.size() == 0}">
    <table style="background-color: white; border-radius: 15px; font-size: 20px; width: 100%">
        <tr>
            <td align="center" style="width: 33%; font-weight: bolder">Нет заказов</td>
        </tr>
    </table>
</c:if>
<br>
<c:forEach var="order" items="${requestScope.reservedOrders}">
    <table style="background-color: white; border-radius: 15px; font-size: 20px; width: 100%">
        <form action="${pageContext.request.contextPath}/reserved-orders" method="post">
            <tr>
